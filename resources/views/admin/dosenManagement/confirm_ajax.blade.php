...
@else
    <form action="{{ route('admin.dosenManagement.destroyAjax', $dosen->id) }}" method="POST" id="form-delete">
        @csrf
        @method('DELETE')
        <div id="modal-master" class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body p-4">
                    <div class="alert alert-warning">
                        <div class="d-flex align-items-center">
                            ...
                            <div>
                                ...
                                <p class="mb-0">Apakah Anda yakin ingin menghapus dosen berikut?</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card mt-4">
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6">
                                    <p class="mb-1 font-weight-bold text-primary">Nama Dosen:</p>
                                    <h5>{{ $dosen->dosen_nama }}</h5>
                                </div>
                                <div class="col-md-6">
                                    <p class="mb-1 font-weight-bold text-info">Username:</p>
                                    <h5>{{ $dosen->user->username }}</h5>
                                </div>
                                <div class="col-md-6 mt-3">
                                    <p class="mb-1 font-weight-bold text-success">NIP:</p>
                                    <h5>{{ $dosen->dosen_nip }}</h5>
                                </div>
                                <div class="col-md-6 mt-3">
                                    <p class="mb-1 font-weight-bold text-success">Program Studi:</p>
                                    <h5>{{ $dosen->prodi ? $dosen->prodi->prodi_nama : '-' }}</h5>
                                </div>
                                <div class="col-md-6 mt-3">
                                    <p class="mb-1 font-weight-bold text-success">Jenis Kelamin:</p>
                                    <h5>{{ $dosen->dosen_gender == 'Laki-laki' ? 'Laki-laki' : 'Perempuan' }}</h5>
                                </div>
                                <div class="col-md-6 mt-3">
                                    <p class="mb-1 font-weight-bold text-warning">Nomor Telepon:</p>
                                    <h5>{{ $dosen->dosen_nomor_telepon }}</h5>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-3 alert alert-danger">
                        <i class="fas fa-exclamation-circle mr-2"></i> Menghapus dosen ini akan menonaktifkan akun pengguna
                            ...
                            $.ajax({
                                ...
                                success: function(response) {
                                    if (response.status) {
                                        ...
                                        const isShowPage = $(
                                            '.card-header:contains("Detail Dosen")'
                                        ).length > 0;
                                        ...
                                        if (isShowPage) {
                                            ...
                                            Swal.fire({
                                                ...
                                                willClose: () => {
                                                    ...
                                                    window.location
                                                        .href =
                                                        "{{ route('admin.dosenManagement.index') }}";
                                                }
                                            });
                                        } else {
